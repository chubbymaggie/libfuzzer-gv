...
{
    public:
        AllocationTracker(size_t maxAlloc)
        {
            this->allocations = new struct Allocation[maxAlloc];
        ...
        void addMalloc(void* ptr, size_t num)
        {
            this->add(Allocation::ALLOC_MALLOC, ptr, num);
        }

        void addFree(void* ptr)
        {
            this->add(Allocation::ALLOC_FREE, ptr, 0);
        }
        ...
        void calcMax(void)
        {
            size_t curAlloc = 0, maxAlloc = 0;

            for (size_t i = 0; i < this->idx; i++)
            {
                switch (this->allocations[i].type)
                {
                    case    Allocation::ALLOC_MALLOC:
                        curAlloc += this->allocations[i].size;
                        if (curAlloc > maxAlloc)
                            maxAlloc = curAlloc;
                        break;
                    case    Allocation::ALLOC_FREE:
                        curAlloc -= this->allocations[i].size;
                        break;
                    default:
                        /* Should not happen */
            ...
            if (maxAlloc > this->allTimeMax) {
                this->allTimeMax = maxAlloc;
            }
        }
    private:
        void add(enum Allocation::AllocType type, void* ptr, size_t size)
        {
            if ( this->idx >= this->maxAlloc ) {
                Printf("Too many allocations\n");
                abort();
            }

            if (type == Allocation::ALLOC_FREE)
            {
                bool found = false;

                for (size_t i = 0; i < this->idx; i++)
                {
                    if (this->allocations[i].type == Allocation::ALLOC_MALLOC)
                        if (this->allocations[i].ptr == ptr) {
                            found = true;
                            size = this->allocations[i].size;
                            break;
                        }
                }
            ...
            this->allocations[this->idx].type = type;
            this->allocations[this->idx].ptr = ptr;
            this->allocations[this->idx].size = size;

            this->idx++;
        }
        struct Allocation* allocations;
        size_t maxAlloc, idx, allTimeMax;
...
void MallocHook(const volatile void *ptr, size_t size) {
  ...
  F->HandleMalloc(size);
  if (optAllocGuided)
      allocationTracker->addMalloc((void*)ptr, size);
...
void FreeHook(const volatile void *ptr) {
  ...
  if (optAllocGuided)
      allocationTracker->addFree((void*)ptr);
  if (int TraceLevel = AllocTracer.TraceLevel) {
    Printf("FREE[%zd]   %p\n", N, ptr);
    if (TraceLevel >= 2 && EF)
      EF->__sanitizer_print_stack_trace();
...
void Fuzzer::PrintStats(const char *Where, const char *End, size_t Units) {
  ...
  if (Options.AllocGuided)
      if (size_t N = allocationTracker->getMax())
          Printf(" a: %zd", N);
  if (Options.CustomGuided)
      if (size_t N = TPC.GetCustomRecord())
          Printf(" cu: %zd", N);
